...
import {AuthRequest, useUserRegisterMutation} from "@/lib/features/auth/authApiSlice";
import {useRouter} from "next/navigation";
import React from "react";


export default function UserRegisterForm(){
    const [userRegisterApi,userRegisterResult] = useUserRegisterMutation();

    const {register,handleSubmit,reset,formState:{errors}} = useForm<UserSchema>({resolver:zodResolver(userSchema)});

    const router = useRouter();

    const submitUserRegisterFormHandler = (data:AuthRequest) => {

        userRegisterApi(data).unwrap().then(data=>{
            reset();
            router.back();
        })
    }
    return(
        <div>
            <form onSubmit={handleSubmit(submitUserRegisterFormHandler)}
